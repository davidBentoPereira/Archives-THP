...
def like_tweets_containing_text(text, number)
  client = login_twitter
  client.search("#bonjour_monde", result_type: "recent").take(number).collect do |tweet|
...
def perform(journalists)
  # Init Twitter API
  client = login_twitter ? continue : exit

  # Put a like on the last 25 tweets containing #bonjour_monde
  like_tweets_containing_text(text, 25)
  # Follow them
end
