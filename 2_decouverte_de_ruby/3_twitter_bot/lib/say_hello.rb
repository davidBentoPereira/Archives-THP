...
def login_twitter
  ...
  client.update("My first Tweet ! #BentoIsInThePlace")
end

puts ENV["TWITTER_CONSUMER_KEY"]

def pick_random_elemts_in_array(array, number)
  final_arr = []
  number.times do |i|
    final_arr[i] = array.sample
  end
  final_arr
end

def tweet_say_hello_to_people(people)
  # Connect to Tweeter
  tweeter_client = login_twitter
  # Select last people
  tweeter_client.update('Mon premier tweet en Ruby !!!!')
  # people.each do |handle|
  #   tweeter_client.update("Hey #{handle}, just wanted to say I love what you're doing !")
  #   # puts "Hey #{handle}, just wanted to say I love what you're doing !"
  # end
end

# def perform(journalists)
#   # Select 5 random handles in JOURNALISTS
#   people = pick_random_elemts_in_array(journalists, 5)
#
#   # Tweet a message to every Journalist
#   tweet_say_hello_to_people(people)
# end

# perform(journalists)

# tweet_say_hello_to_people(journalists)
login_twitter